...
def main():
    ...
    wav_path = saving_path[:saving_path.rfind(".")] + ".wav"
    if saving_path.endswith(".mp3"):
        convert_mp3_to_wav(saving_path, wav_path)
    elif saving_path.endswith(".mp4"):
        convert_mp4_to_wav(saving_path, wav_path)
    elif saving_path.endswith(".wav"):
        pass
    else:
        raise ValueError("Unknown file extension!")
    base_transcript_path = automatic_speech_recognition(wav_path)
    speakers_transcript_path = speaker_identification(wav_path)
    transcript_analysis(base_transcript_path, speakers_transcript_path)
    transcript_quality_improvement_comparison(wav_path, base_transcript_path,
                                              speakers_transcript_path)
