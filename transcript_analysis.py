...
def interactive_timeline():
    # you can press and listen certain bits
    pass


def annotate_transcript_problem(transcripts, output_file):
    # saves on the disk a user correction of the transcript
    with open(output_file, 'w') as json_file:
        json.dump(transcripts, json_file, indent=4)



def speaker_diarization_from_video(frames):
    # based on who the camera sees, or just from
    # in reality it's not that important because people are already registered in a
    # platform with their mail, username, everything there, it's not like they have
    # different all a common microphone
